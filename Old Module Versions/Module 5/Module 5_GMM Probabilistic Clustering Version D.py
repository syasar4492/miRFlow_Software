...
#
# Interpretation:
#   diagnostic_cofactor = expected_sandwich_total of numerator bead /
#                         expected_sandwich_total of denominator bead
#
...
def build_diagnostic_rows(df_master: pd.DataFrame, sample_order: List[str]) -> pd.DataFrame:
    ...
    for sample in sample_order:
        ...
        for bead in (3, 6, 8):
            ...
            if bead_row.empty:
                bead_totals[bead] = np.nan
            else:
                bead_totals[bead] = float(bead_row.iloc[0]["expected_sandwich_total"])
        ...
        for ratio_label, numerator, denominator in RATIO_ORDER:
            num = bead_totals.get(numerator, np.nan)
            den = bead_totals.get(denominator, np.nan)
