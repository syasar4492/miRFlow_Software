...
#
#   5) Produces plots:
#       - 2D scatter for every file: fluor vs FSC
#       - MMB files: only 1D FSC/size density plots with local size boundaries
#       - DG files: only 1D fluorescence density plots with local DG windows
#
...
#
# =============================================================================
# IMPORTANT NOTE: WHAT WAS FIXED PREVIOUSLY
# =============================================================================
# The global MMB FSC window is computed ONLY from MMB files.
# The global DG fluorescence window is computed ONLY from DG files.
#
# =============================================================================
...
#
# This preserves the original low/background exclusion while expanding the upper
# fluorescence window to avoid prematurely excluding viable events.
# =============================================================================
...
def dg_fluoro_window_bio_aligned(values: np.ndarray, shoulder_frac: float = 0.05):
    ...
    # -------------------------------------------------------------
    # (1) High peak = KDE global max (dominant high/free-DG population)
    # -------------------------------------------------------------
    ...
    # This is NOT the final high_cut anymore.
    # It is retained as the anchor point for the original low_cut logic.
    # -------------------------------------------------------------
    left_shoulder_idx = high_peak
    while left_shoulder_idx > 0 and ys[left_shoulder_idx] > high_thr:
        left_shoulder_idx -= 1

    left_shoulder_for_low_search = float(xs[left_shoulder_idx])

    # -------------------------------------------------------------
    # (2b) NEW high_cut = RIGHT shoulder of high peak using the 5% rule
    # -------------------------------------------------------------
    ...
    # -------------------------------------------------------------
    # (3) Find a meaningful bump on the left side for low_cut
    # Search is restricted to the LEFT of the dominant peak's LEFT shoulder.
    # -------------------------------------------------------------
...
def process_pattern(folder: Path, pattern: str, base_out_dir: Path):
    ...
    for f in files:
        ...
        # -------------------------------------------------
        # Per-file FSC window computed for all files for record-keeping,
        # but only MMB files contribute to global MMB window and only
        # MMB files generate 1D size distribution plots.
        # -------------------------------------------------
        ...
        # -------------------------------------------------
        # Per-file DG fluorescence window computed for DG files only
        # -------------------------------------------------
        dg_window = None
        if is_dg:
    ...
    # Global DG window computed only from DG files
    ...
    summary = {
        ...
        "dg_window_definition": {
            ...
            "implementation_notes": {
                ...
                "plotting_change": (
                    "MMB files generate 1D size plots only; DG files generate 1D fluorescence plots only."
                )
            }
        }
    }
...
def main():
    folder = Path(
        r"D:\ICL MBE\Year 4\FYP\Software Automation\Raw Flow Data\MMB & DG Controls Unrefined"
    )
