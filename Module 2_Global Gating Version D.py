...
#
#      B) DG fluorescence low_cut:
#         - Biological goal: define the lower fluorescence boundary enriched for
#           potentially useful/functionalised DG-associated events while excluding
...
#
#         - The previous control-derived DG high_cut is no longer exported because
#           the high-fluorescence tail becomes exaggerated in patient/plasma samples.
#           Instead, the downstream Module 2.5 will create patient-specific adaptive
#           piecewise high boundaries.
#
#         - A fixed DG peak fluorescence anchor is exported as:
...
#       - DG files: 1D fluorescence density plots with local DG low_cut.
#
#   6) Writes a global summary JSON:
#       - mmb_window aggregated across MMB files only:
#           [min(left_i), max(right_i)]
#       - dg_low_cut aggregated across DG files only:
#           GLOBAL_LOW = min(low_cut_i)
#       - dg_peak_fluorescence:
#           fixed anchor value for downstream Module 2.5 piecewise gating.
#
# =============================================================================
...
# =============================================================================
# USER-DEFINED DG PEAK FLUORESCENCE ANCHOR
# =============================================================================
# This value represents the approximate fluorescence value of the dominant DG
# high/free-DG density peak after Logicle transformation.
#
# It is exported into gate_summary_by_type.json and will be used by Module 2.5
# to construct patient-specific adaptive piecewise fluorescence limits.
# =============================================================================
...
# =============================================================================
# 6) DG logic: fluorescence low_cut only
# =============================================================================
# The previous Module 2 version calculated a full DG fluorescence window:
#   [low_cut, high_cut]
#
# For the new piecewise gating strategy, only the DG low_cut is exported from
# Module 2. The upper fluorescence boundary will be generated later by Module 2.5
# using patient-specific 2D piecewise gating.
#
...
#
# The dominant DG peak fluorescence anchor is not calculated here yet.
# It is currently exported as the fixed value:
#   DG_PEAK_FLUORESCENCE = 0.800000000000
# =============================================================================

def dg_fluoro_low_cut_bio_aligned(values: np.ndarray, shoulder_frac: float = 0.05):
    xs, ys = kde_density(values, n=2048)
    v = values[np.isfinite(values)]

    # If KDE is unreliable, return a conservative low fluorescence boundary
...
def process_pattern(folder: Path, pattern: str, base_out_dir: Path):
    ...
    # Global DG low_cut uses DG files only
    dg_low_cuts = []
    ...
    for f in files:
        ...
        # -------------------------------------------------
        # Per-file DG fluorescence low_cut
        # Only DG files contribute to the global DG low_cut.
        # Only DG files generate 1D fluorescence plots.
        # -------------------------------------------------
        dg_low_cut = None

        if is_dg:
            dg_low_cut = dg_fluoro_low_cut_bio_aligned(
                df_t[fluor_name].to_numpy(),
                shoulder_frac=0.05
            )

            dg_low_cuts.append(dg_low_cut)
        ...
        # ----------------------
        # DG only: 1D fluorescence plot with local DG low_cut
        # ----------------------
        if is_dg:
    ...
    if dg_low_cuts:
        global_dg_low_cut = float(min(dg_low_cuts))
    else:
        global_dg_low_cut = None
    ...
    summary = {
        ...
        "dg_window_definition": {
            "method": "bio_aligned_low_cut_only_with_fixed_peak_anchor",
            "shoulder_frac": 0.05,
            "interpretation": (
                "Module 2 now exports only the lower DG fluorescence boundary. "
                "The upper fluorescence boundary is intentionally not exported here "
                "because it will be generated per patient in Module 2.5 using "
                "piecewise 2D polygon gating."
            ),
            "implementation_notes": {
                ...
                "dg_peak_fluorescence": (
                    ...
                    "this will be used downstream by Module 2.5 to build adaptive "
...
def main():
    ...
    base_out_dir = Path(
        r"D:\ICL MBE\Year 4\FYP\Software Automation\Pre Term Birth\Controls Refined"
    )
