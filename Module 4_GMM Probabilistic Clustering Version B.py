...
OUTPUT_ROOT = r"D:\ICL MBE\Year 4\FYP\Software Automation\Visual Flow Data\20 samples optimised (FlowJo axes)"
...
# Display mode for cluster plots / overview grid
# "FLOWJO_LIKE" = raw values on log axes when possible
# "TRANSFORMED" = old transformed-axis display
DISPLAY_MODE = "FLOWJO_LIKE"
...
try:
    ...
    try:
        from flowutils.transforms import logicle_inverse as logicle_inverse_transform
    except Exception:
        logicle_inverse_transform = None
except Exception:
...
def get_transform_params(arr: np.ndarray) -> dict:
    """
    Return the transform parameters that would be used for this array.
    Needed so displayed gate lines can be mapped back to raw scale.
    """
...
def inverse_transform_values(values_t: np.ndarray, raw_reference: np.ndarray) -> Optional[np.ndarray]:
    """
    Map transformed values back to raw scale using the same transform-parameter rule.
    Returns None if inverse logicle is unavailable.
    """
    vals = np.asarray(values_t, dtype=float)
    params = get_transform_params(raw_reference)

    if params["mode"] == "logicle":
        if logicle_inverse_transform is None:
            return None
        return logicle_inverse_transform(
            vals,
    ...
    # asinh fallback inverse
    return np.sinh(vals) * params["cofactor"]
...
def prepare_plot_arrays(
    ...
    labels: np.ndarray,
    display_mode: str,
    require_raw_lines: bool = False,
    raw_lines_ready: bool = False,
):
    """
    Decide whether to plot raw-log (FlowJo-like) or transformed values.
    """
    if display_mode == "FLOWJO_LIKE":
        if (not require_raw_lines) or raw_lines_ready:
            m = np.isfinite(x_raw) & np.isfinite(y_raw) & (x_raw > 0) & (y_raw > 0)
            x_plot = x_raw[m]
            y_plot = y_raw[m]
            lab_plot = labels[m]
            if x_plot.size >= 50 and y_plot.size >= 50:
                return x_plot, y_plot, lab_plot, True

    # fallback to transformed view
    m = np.isfinite(x_t) & np.isfinite(y_t)
    return x_t[m], y_t[m], labels[m], False


def plot_patient_clusters(
    out_path: Path,
    fluor_raw: np.ndarray,
    fsc_raw: np.ndarray,
    fluor_t: np.ndarray,
    fsc_t: np.ndarray,
    ...
    transform_label: str,
    title: str,
    dg_window_t: Tuple[float, float],
    mmb_window_t: Tuple[float, float],
    b36_t: float,
    b68_t: float,
    dg_window_raw: Optional[Tuple[float, float]],
    mmb_window_raw: Optional[Tuple[float, float]],
    b36_raw: Optional[float],
    b68_raw: Optional[float],
    display_mode: str,
):
    raw_lines_ready = (
        dg_window_raw is not None and
        mmb_window_raw is not None and
        b36_raw is not None and
        b68_raw is not None
    )

    x, y, lab, use_log_axes = prepare_plot_arrays(
        fluor_raw, fsc_raw, fluor_t, fsc_t, cluster_labels,
        display_mode=display_mode,
        require_raw_lines=True,
        raw_lines_ready=raw_lines_ready,
    )

    # Downsample for readability
    x, y, lab = downsample_xy(x, y, lab, MAX_POINTS_PER_PLOT, seed=0)
    ...
    if use_log_axes:
        style_log_axes(plt.gca())

        # plot raw-scale gates / boundaries
        dg_low, dg_high = dg_window_raw
        mmb_low, mmb_high = mmb_window_raw
        plt.axvline(dg_low, ls="--", lw=1)
        plt.axvline(dg_high, ls="--", lw=1)
        plt.axhline(mmb_low, ls="--", lw=1)
        plt.axhline(mmb_high, ls="--", lw=1)
        plt.axhline(b36_raw, ls=":", lw=1)
        plt.axhline(b68_raw, ls=":", lw=1)

        plt.xlabel(fluor_name)
        plt.ylabel(fsc_name)
    else:
        # transformed display fallback
        dg_low, dg_high = dg_window_t
        mmb_low, mmb_high = mmb_window_t
        plt.axvline(dg_low, ls="--", lw=1)
        plt.axvline(dg_high, ls="--", lw=1)
        plt.axhline(mmb_low, ls="--", lw=1)
        plt.axhline(mmb_high, ls="--", lw=1)
        plt.axhline(b36_t, ls=":", lw=1)
        plt.axhline(b68_t, ls=":", lw=1)

        plt.xlabel(f"{fluor_name} ({transform_label})")
        plt.ylabel(f"{fsc_name} ({transform_label})")
...
def save_grid_overview(
    ...
    """
    panels: list of dict with keys {name, x_raw, y_raw, x_t, y_t, labels}
    """
    n = len(panels)
    if n == 0:
        return
    ...
    for idx, p in enumerate(panels):
        ...
        x, y, lab, use_log_axes = prepare_plot_arrays(
            p["x_raw"], p["y_raw"], p["x_t"], p["y_t"], p["labels"],
            display_mode=display_mode,
            require_raw_lines=False,
            raw_lines_ready=False,
        )
        ...
        if use_log_axes:
            style_log_axes(ax)
            ax.set_xlabel(fluor_name, fontsize=8)
            ax.set_ylabel(fsc_name, fontsize=8)
        else:
            ax.set_xlabel(f"{fluor_name} ({transform_label})", fontsize=8)
            ax.set_ylabel(f"{fsc_name} ({transform_label})", fontsize=8)
...
def main():
    ...
    for f in fcs_files:
        ...
        # Raw values
        fluor_raw_all = df[fluor_name].to_numpy(dtype=float)
        fsc_raw_all = df[fsc_name].to_numpy(dtype=float)

        # Transform
        fluor_t = apply_logicle_or_asinh(fluor_raw_all)
        fsc_t = apply_logicle_or_asinh(fsc_raw_all)

        # Finite filter
        finite = np.isfinite(fluor_t) & np.isfinite(fsc_t)
        fluor_t = fluor_t[finite]
        fsc_t = fsc_t[finite]
        fluor_raw = fluor_raw_all[finite]
        fsc_raw = fsc_raw_all[finite]
        ...
        fsc_roi = fsc_t[roi]
        fluor_raw_roi = fluor_raw[roi]
        fsc_raw_roi = fsc_raw[roi]
        ...
        # --- Convert gate/boundary lines back to raw scale for FlowJo-like display ---
        dg_window_raw_disp = None
        mmb_window_raw_disp = None
        b36_raw_disp = None
        b68_raw_disp = None

        if DISPLAY_MODE == "FLOWJO_LIKE":
            dg_inv = inverse_transform_values(np.array(dg_window, dtype=float), fluor_raw)
            mmb_inv = inverse_transform_values(np.array(mmb_window, dtype=float), fsc_raw)
            bead_inv = inverse_transform_values(np.array([b36, b68], dtype=float), fsc_raw)

            if dg_inv is not None and mmb_inv is not None and bead_inv is not None:
                if (np.all(np.isfinite(dg_inv)) and np.all(np.isfinite(mmb_inv)) and np.all(np.isfinite(bead_inv)) and
                        np.all(dg_inv > 0) and np.all(mmb_inv > 0) and np.all(bead_inv > 0)):
                    dg_window_raw_disp = (float(dg_inv[0]), float(dg_inv[1]))
                    mmb_window_raw_disp = (float(mmb_inv[0]), float(mmb_inv[1]))
                    b36_raw_disp = float(bead_inv[0])
                    b68_raw_disp = float(bead_inv[1])

        # --- Save per-patient plot ---
        transform_label = "logicle" if LOGICLE_SOURCE != "asinh-fallback" else "asinh"
        plot_path = plots_dir / f"{safe_filename(stem)}_clusters_{transform_label}_{DISPLAY_MODE}.png"

        plot_patient_clusters(
            out_path=plot_path,
            fluor_raw=fluor_raw_roi,
            fsc_raw=fsc_raw_roi,
            fluor_t=fluor_roi,
            fsc_t=fsc_roi,
            ...
            dg_window_t=dg_window,
            mmb_window_t=mmb_window,
            b36_t=b36,
            b68_t=b68,
            dg_window_raw=dg_window_raw_disp,
            mmb_window_raw=mmb_window_raw_disp,
            b36_raw=b36_raw_disp,
            b68_raw=b68_raw_disp,
            display_mode=DISPLAY_MODE,
        )
    ...
    if grid_panels:
        transform_label = "logicle" if LOGICLE_SOURCE != "asinh-fallback" else "asinh"
        grid_path = out_root / f"ALL_PATIENTS_GRID_9clusters_{transform_label}_{DISPLAY_MODE}.png"

        save_grid_overview(
            out_path=grid_path,
            panels=grid_panels,
            fluor_name=(fluor_pref or "Fluor"),
            fsc_name=(fsc_pref or "FSC"),
            transform_label=transform_label,
