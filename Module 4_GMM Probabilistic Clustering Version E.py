...
JSON_DIR = r"D:\ICL MBE\Year 4\FYP\Software Automation\Pre Term Birth\Visual Flow Data\Json Files"
...
# =============================================================================
# Fill these in later when you know the EC thresholds.
#
# Format:
#   "MMB 3:6": {"mean": 1.20, "sd": 0.15}
#
# Interpretation:
#   diagnostic_cofactor = normalised_sandwich_events (%) of numerator bead /
#                         normalised_sandwich_events (%) of denominator bead
#
# The normalised value is calculated in the main summary sheet as:
#   100 * expected_sandwich_total / (expected_sandwich_total + expected_only)
#
# EC_probability logic:
#   within ±1 SD = high      -> red    -> score 2
#   within ±2 SD = moderate  -> yellow -> score 1
#   outside ±2 SD = low      -> green  -> score 0
#
...
EC_THRESHOLDS = {
    "MMB 3:6": {"mean": 0.5, "sd": 0.15},
    "MMB 3:8": {"mean": 1.2, "sd": 0.20},
    "MMB 6:3": {"mean": 0.64, "sd": 0.10},
    "MMB 6:8": {"mean": 0.85, "sd": 0.15},
    "MMB 8:3": {"mean": 0.55, "sd": 0.12},
    "MMB 8:6": {"mean": 0.70, "sd": 0.18},
}
...
def classify_ec_probability(ratio_label: str, diagnostic_cofactor):
    """
    Returns:
      EC_threshold_display, EC_probability, risk_score
    ...
    if z <= 1:
        return threshold_display, "high", 2
    elif z <= 2:
        return threshold_display, "moderate", 1
    else:
        return threshold_display, "low", 0




def calculate_normalised_sandwich_percent(expected_sandwich_total, expected_only):
    """
    Calculate the normalised sandwich signal for one bead population.

    Formula used for the Excel column:
      normalised_sandwich_events (%) =
          100 * expected_sandwich_total / (expected_sandwich_total + expected_only)

    Why this is useful:
      - expected_sandwich_total alone is an absolute count and can be affected by
        the number of events assigned to each bead population.
      - normalising by expected_sandwich_total + expected_only asks what fraction
        of informative bead events are sandwich-positive.
      - Diagnostic cofactors are then calculated as ratios of these normalised
        values between MMB classes.
...
def build_diagnostic_rows(df_master: pd.DataFrame, sample_order: List[str]) -> pd.DataFrame:
    ...
    Important:
      Diagnostic cofactors are calculated from normalised_sandwich_events (%)
      rather than raw expected_sandwich_total counts.

      Example:
        MMB 3:6 = normalised MMB-3 / normalised MMB-6
    """
    rows = []
    ...
    for sample in sample_order:
        ...
        # Store the normalised sandwich percentage for each bead population.
        # These values, rather than raw expected_sandwich_total counts, are used
        # to calculate the diagnostic_cofactor ratios below.
        bead_normalised = {}

        for bead in (3, 6, 8):
            bead_row = sample_df[sample_df["bead"].astype(str) == str(bead)]
            if bead_row.empty:
                bead_normalised[bead] = np.nan
            else:
                bead_normalised[bead] = float(bead_row.iloc[0]["normalised_sandwich_events (%)"])
        ...
        for ratio_label, numerator, denominator in RATIO_ORDER:
            # Diagnostic cofactor = ratio of normalised sandwich percentages.
            # Example: MMB 3:6 = normalised_sandwich_events (%) of MMB-3 /
            #                  normalised_sandwich_events (%) of MMB-6.
            num = bead_normalised.get(numerator, np.nan)
            den = bead_normalised.get(denominator, np.nan)

            if not np.isfinite(num) or not np.isfinite(den) or den == 0:
                diagnostic_cofactor = ""
            else:
                diagnostic_cofactor = num / den

            threshold_display, probability, score = classify_ec_probability(
                ratio_label,
                diagnostic_cofactor
            )
            ...
            rows.append({
                ...
                "diagnostic_cofactor": diagnostic_cofactor,
                "EC_threshold": threshold_display,
                "EC_probability": probability,
                "Overall Risk Score (12)": score,
                "_row_type": "ratio"
            })
        ...
        rows.append({
            ...
            "Overall Risk Score (12)": overall_score,
...
def main():
    ...
    for f in fcs_files:
        ...
        for b in (3, 6, 8):
            ...
            if res.model_kind == "skipped" or res.p_only is None:
                per_bead_summary[str(b)] = {
                    ...
                    "expected": {
                        ...
                        "sandwich_v3": None,
                        "sandwich_total": None,
                        "normalised_sandwich_events_percent": None,
                    },
                    "warning": res.warn,
                }
                continue
            ...
            # Normalised sandwich signal for this bead population.
            # This is exported to the Excel summary sheet and then used to build
            # the Diagnostic Metrics ratios.
            normalised_sandwich_percent = calculate_normalised_sandwich_percent(
            ...
            per_bead_summary[str(b)] = {
                ...
                "expected": {
                    ...
                    "normalised_sandwich_events_percent": normalised_sandwich_percent,
                },
                "warning": res.warn,
            }
            ...
            master_rows.append({
                ...
                "expected_sandwich_total": e_sandwich_total,
                "normalised_sandwich_events (%)": normalised_sandwich_percent,
            })
        ...
        # Overall normalised sandwich signal across all bead populations in the sample.
        # This is included for completeness in the ALL row, but diagnostic cofactors
        # are calculated using the per-bead normalised values above.
        total_normalised_sandwich_percent = calculate_normalised_sandwich_percent(
        ...
        master_rows.append({
            ...
            "normalised_sandwich_events (%)": total_normalised_sandwich_percent,
        })
